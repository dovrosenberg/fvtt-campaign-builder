//import { MonksEnhancedJournal, error, setting, getVolume, isV11  } from "../monks-enhanced-journal.js"
...
export class WorldBuilder extends Application {
	private _tabList = [] as WindowTab[];  // this is strange object that looks like an array plus an "active"
	bookmarks = [];
	searchresults = [];
	searchpos = 0;
	lastquery = '';
	...
	constructor(/*object, options = {}*/) {
		...
		this._tabList = duplicate(getGame().user?.getFlag('world-builder', 'tabs') || 
				[{ id: randomID(), text: localize('fwb.labels.newTab'), active: true, history: [] }]) as WindowTab[];
		this._tabList = this._tabList.map(t => { delete t.entity; return t; })
		//this.bookmarks = duplicate(getGame().user?.getFlag('world-builder', 'bookmarks') || []);
	...
	private _activeTab(findone = true): WindowTab | null {
		let tab = this._tabList.find(t => t.active);
		if (findone) {
			if (!tab && this._tabList.length > 0)  // nothing was marked as active, just pick the 1st one
				tab = this._tabList[0];
		}
