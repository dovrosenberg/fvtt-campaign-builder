...
const DirectoryScrollService = {
   ...
   */
  scrollToActiveEntry: async (): Promise<void> => {
    ...
    switch (currentTab.tabType) {
      case WindowTabType.Entry:
        await DirectoryScrollService.scrollToEntry(contentId);
        break;
      case WindowTabType.Campaign:
        await DirectoryScrollService.scrollToCampaign();
        break;
      case WindowTabType.Session:
        await DirectoryScrollService.scrollToSession(contentId);
        break;
      case WindowTabType.Setting:
        await DirectoryScrollService.scrollToSetting();
        break;
      case WindowTabType.Front:
        await DirectoryScrollService.scrollToFront(contentId);
        break;
      case WindowTabType.Arc:
        await DirectoryScrollService.scrollToArc(contentId);
        break;
      default:
        return;
    }
   ...
   * @returns A promise that resolves when the scroll operation is complete
   */
  scrollToEntry: async (entryId: string): Promise<void> => {
    const mainStore = useMainStore();
    ...
    await DirectoryScrollService.scrollToElement('.fcb-current-directory-entry');
  },
   ...
   * @returns A promise that resolves when the scroll operation is complete
   */
  scrollToSession: async (sessionId: string): Promise<void> => {
   ...
   * @returns A promise that resolves when the scroll operation is complete
   */
  scrollToFront: async (frontId: string): Promise<void> => {
    const campaignDirectoryStore = useCampaignDirectoryStore();
   ...
   * @returns A promise that resolves when the scroll operation is complete
   */
  scrollToArc: async (arcId: string): Promise<void> => {
    const campaignDirectoryStore = useCampaignDirectoryStore();
   ...
   */
  scrollToElement: async (selector: string): Promise<void> => {
    const element = document.querySelector(selector) as HTMLElement;
    
    if (!element || DirectoryScrollService.isActiveEntryVisible(selector)) 
      return;
   ...
   */
  isActiveEntryVisible: (selector: string): boolean => {
