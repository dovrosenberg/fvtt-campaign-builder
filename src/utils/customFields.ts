...
const CustomFieldsService = {
  makeCustomFieldKeyUnique(baseKey: string, usedKeys: Set<string>): string {
    let key = baseKey;
   ...
   */
  resetDefaultCustomFields: async () => {
    ...
    await CustomFieldsService.initializeDefaultImageConfigurations(boxedTextKey);
  },

  /** 
   * Set the default image configurations for the first time.
   */
  initializeDefaultImageConfigurations: async (boxedTextKey: string) => {
  ...
  windowTabToCustomContentType: (windowTabType: WindowTabType, entry?: any) => {
