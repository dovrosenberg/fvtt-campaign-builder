...
When you save an editor, if you have exactly (but case insensitive) used the name of any [^Entry], [^PC], or [^Session], it will automatically turn it into a reference link.  

> [!Note]
> If you have more than one Entry, PC, and/or Session with the same name, it will not create a link, so that it's not linking to the wrong one.  Instead, use Drag & Drop or Autocomplete Mentions to insert a reference to the one you want.  This is likely to be a very rare situation, though.
