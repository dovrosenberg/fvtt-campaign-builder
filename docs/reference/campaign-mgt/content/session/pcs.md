---
title: Session PCs Tab
prev: 
  text: 'Session Magic Items Tab'
  link: '/reference/campaign-mgt/content/session/magic-items'
next: 
  text: 'To-do List'
  link: '/reference/campaign-mgt/content/campaign/todos'
---
# Session PCs Tab
![Session PCs Tab](/assets/images/session-pc-tab.webp)

The Session PCs tab is an exact duplicate of the [Campaign PCs tab](/reference/campaign-mgt/content/campaign/pcs).  It is provided on the Session screen solely for ease of use during prep/play.
